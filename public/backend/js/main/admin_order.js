        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(k, v) {
                    ...
                    <td>${k+1}</td>
                    <td>${v.order_code}</td>
                    <td> ${numberWithCommas(v.order_total_cost)} đ</td>`;
    ...
    function phantrang(trang) {
        ...
        for (let i = t; i < n; i++) {
            ...
            output += `
                ...
                <td> ${numberWithCommas(arr_tam[i].order_total_cost)} đ</td>`;
            ...
            output += `<td> ${arr_phantrang[i].order_created}</td>
                <td>`;
            if (arr_phantrang[i].order_status == 1 || arr_phantrang[i].order_status == 2 || arr_phantrang[i].order_status == 3)
                ...
                $.each(data.data, function(k, v) {
                    ...
                    $.each(v.detail, function(j, d) {
                        ...
                        if (d.detail_status == 'Y' || d.detail_status == 'N') {
                            ...
                            output += `<tr>
                                        <td colspan ="2"><h5>${d.product_title} (${numberWithCommas(d.product_sales_price)} đ)</h5>
                            ...
                            tongtam = d.detail_cost * d.detail_quantity;
                            tongcuoi += tongtam;
                            output += `     <div>${numberWithCommas(d.detail_cost)} x  ${d.detail_quantity} = ${numberWithCommas(tongtam)} đ</div>
                                        </td>
                                        </tr>
                                    `;
                        }
                    ...
                    if (v.order_status == 1 || v.order_status == 2) {
                        ...
                        <tr>
                            ...
                            <td style="color:#000000"><h3>${numberWithCommas(tongcuoi)} đ</h3></td>
                        </tr>
                        ...
                        <tr style="color:Green">
                            ...
                            <td><h2>${numberWithCommas(tongcuoi)} đ</h2></td>
                        </tr>
                    </table>`;
                    } else {
                        output += `
                        ...
                        <tr>
                            <td style="color:Blue"><h3>Tổng tạm tính</h3></td>
                            <td style="color:#000000"><h3>${numberWithCommas(Number(v.order_total_cost)+Number(v.order_direct_discount))} đ</h3></td>
                        </tr>
                        ...
                        <tr style="color:Green">
                            <td><h2>Thành tiền</h2></td>
                            <td><h2>${numberWithCommas(Number(v.order_total_cost))} đ</h2></td>
                        </tr>
                    </table>`;
                    }
    ...
    function seach_code_order() {
        var output = "";
        $.ajax({
            url: "../admin/order-order-seach-code",
            method: "post",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token-seach-code"]').attr('content')
            },
            data: { keyseach: $('#key_seach_code').val() },
            dataType: 'JSON',
            success: function(data) {
                arr_phantrang = data;
                $.each(data, function(k, v) {
